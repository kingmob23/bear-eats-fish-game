...
export function createActionButton(
  ...
  const actionButtonWidth =
    scene.textures.get('actionButton').getSourceImage().width * buttonScale;
  const actionButtonHeight =
    scene.textures.get('actionButton').getSourceImage().height * buttonScale;
  ...
  const actionButton = scene.add
    .sprite(finalX, finalY, 'actionButton')
...
export function createSplash(
  ...
  const splash = scene.add.sprite(
    screenWidth / 2,
    splashVerticalOffset,
    'splash',
  );
  splash.setScale(0.4);
